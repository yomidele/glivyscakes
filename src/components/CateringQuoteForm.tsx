...
const eventTypes = ["Wedding", "Birthday Party", "Corporate Event", "Baby Shower", "Funeral Reception", "Other"];
const budgetRanges = ["₦50,000 - ₦100,000", "₦100,000 - ₦250,000", "₦250,000 - ₦500,000", "₦500,000 - ₦1,000,000", "₦1,000,000+"];

const CateringQuoteForm = () => {
  const [formData, setFormData] = useState({
    fullName: "", phone: "", email: "", eventType: "", eventDate: "", numGuests: "", location: "", budgetRange: "", notes: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("catering_quotes").insert({
      full_name: formData.fullName, phone: formData.phone, email: formData.email, event_type: formData.eventType,
      event_date: formData.eventDate, num_guests: parseInt(formData.numGuests), location: formData.location,
      budget_range: formData.budgetRange, notes: formData.notes,
    });
    setIsSubmitting(false);
    if (error) { toast.error("Failed to submit request. Please try again."); }
    else { setSubmitted(true); toast.success("Quote request submitted!"); }
  };
  ...
  if (submitted) {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-lg mx-auto text-center py-12">
        ...
        <p className="text-muted-foreground">Our team will contact you shortly regarding your catering request.</p>
      </motion.div>
    );
  }

  return (
    <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-lg mx-auto space-y-4">
      <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-semibold tracking-wider uppercase text-muted-foreground mb-2">Event Type *</label>
        ...
        <div>
          <label className="block text-xs font-semibold tracking-wider uppercase text-muted-foreground mb-2">Event Date *</label>
          <input name="eventDate" type="date" value={formData.eventDate} onChange={handleChange} required className={inputClass} />
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-semibold tracking-wider uppercase text-muted-foreground mb-2">Number of Guests *</label>
        ...
        <div>
          <label className="block text-xs font-semibold tracking-wider uppercase text-muted-foreground mb-2">Budget Range</label>
      ...
      <div>
        <label className="block text-xs font-semibold tracking-wider uppercase text-muted-foreground mb-2">Location *</label>
